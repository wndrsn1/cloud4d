...
- Stage 1: 2D cloud property prediction (cloud base height, thickness)
...
def make_binary_cloud(occupancy_logits, cloud_base_heights,
    ...
    Constructs a 3D binary cloud field between the predicted cloud base and
    top heights.
    ...
    Returns:
        Coarse cloud volume of shape (B, 1, X, Y, Z) with binary occupancy
    """
    batch_size = occupancy_logits.shape[0]
    ...
    dtype = cloud_base_heights.dtype
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            ...
            curr_cloud = (curr_cloud > 0).to(dtype)
...
class Cloud4D(torch.nn.Module):
    ...
    - Stage 1: 2D cloud property prediction (CBH, thickness)
    - Stage 2: 3D volume refinement using sparse 3D CNNs
    ...
    def forward(self, images, left_plucker_rays, right_plucker_rays, depths, projection_indices, valid_projection_indices, grid_depths, right_images=None,
        ...
        if self.stage == 1:
            occupancy_logits, cloud_base_heights, delta_heights_pred, all_valid_voxels = self.stage1_2d_cloud(
        ...
        coarse_clouds = make_binary_cloud(
            occupancy_logits, cloud_base_heights, delta_heights_pred,
...
class Stage1_2D(torch.nn.Module):
    ...
    def __init__(self, grid_shape, voxel_sizes, grid_ranges, stage):
        ...
        self.height_predictor = UNet(unet_input_dim, out_channels=2, model_channels=128, resolution=200, num_blocks=4, attn_resolutions=[20])
    ...
    def forward(self, projection_indices, all_images, valid_projection_indices):
        ...
        cloud_base_heights, delta_heights_pred = height_preds[:, 0:1, :, :], height_preds[:, 1:2, :, :]
        ...
        return occupany_logits, cloud_base_heights, delta_heights_pred, all_valid_voxels
